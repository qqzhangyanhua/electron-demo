<template>
  <a-spin :spinning="spinning">
    ...
    <a-space class="pl-10" v-if="isSuccess">
      <a-button type="primary" @click="handelClick('vscode')"
        >打开vscode</a-button
      >
      <a-button type="primary" @click="handelClick('iterm')"
        >打开命令行</a-button
      >
      <a-button type="primary" @click="handelClick('finder')"
        >打开所在目录</a-button
      >
    </a-space>
  </a-spin>
...
const handelClick = (type: string) => {
  ipcRenderer.send("scriptExe", { type, filePath: `${formState.filePath}/${formState.projectName}` });
};
</script>
